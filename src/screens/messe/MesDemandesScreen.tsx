...
const MesDemandes = () => {
  ...
  const fetchDemandes = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/messe/mes-demandes`, {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <Text>Date : {item.date_messe}</Text>
      <Text>Heure : {item.heure_messe}</Text>
      <Text>Intention : {item.intentions}</Text>
      <Text style={styles.status}>
        Statut : <Text style={item.status === 'validée' ? styles.valide : styles.attente}>{item.status || 'En attente'}</Text>
      </Text>
    </View>
  );
